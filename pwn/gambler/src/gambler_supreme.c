...
void casino()
{
	...
	while (balance > 0) {
		...
		printf("Guess me a string of length %d with lowercase letters: ", difficulty);
		gets(guess_word);
		...
		if (balance >= 1000) {
			printf("Drats, the cat snuck in and deleted the code for give_flag...");
			break;
		}
	}
...
{
	setup();
	printf("This version comes with a new fancy feature: difficulty!\n");

	difficulty = 0;
	while (difficulty > 7 || difficulty < 1) {
		printf("Enter a difficulty between 1 and 7 (inclusive): ");
		scanf("%d", &difficulty);
		getchar();
	}
	bet_loss *= difficulty;
