...
void symTablePut(ObjString *sym) {
  Value v;
  if (!tableGet(&symbolTable, sym, &v)) {
    char randName[10];
    for (int i = 0; i < 10; i++) {
      randName[i] = 'a' + (rand() % 26);
    }
    ObjString *rename = copyString(randName, 10);
    tableSet(&symbolTable, sym, OBJ_VAL(rename));
  }
}
...
static void number(bool _) {
  ...
  bool negate = false;
  bool floating = false;
  for (const char *c = parser.previous.start;
       c < parser.previous.start + parser.previous.length; c++) {
    if (*c == '.') {
      floating = true;
      break;
    }
...
static void classDeclaration() {
  ...
  Value nameVal = currentChunk()->constants.values[nameConstant];
  symTablePut(AS_STRING(nameVal));
  declareVariable();
